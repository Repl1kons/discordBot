...
# from extenshions.decorator import get_ready
import extenshions.program as param

FFMPEG_OPTIONS = param.settins_music.FFMPEG_OPTIONS
genius = lg.Genius(param.settins_music.token_genius, skip_non_songs=True, excluded_terms=["(Remix)", "(Live)"], remove_section_headers=True)
                    ...
                    if self.voice_client.is_paused() == False:
                        ...
                        if self.query != []:
                            ...
                            self.voice_client.play(FFmpegPCMAudio(source,
                                                                  **FFMPEG_OPTIONS, executable=param.settins_music.executable))
...
class Music(commands.Cog):
    ...
    async def skip(self, context):
        ...
        await context.send("Skip track")
    #
    # @commands.command(name="volume")
    # async def volume(self, ctx, volume: int):
    #     await self.voice_client.set_volume(volume)
    #     await ctx.send(f"set volume: {volume}")
    ...
    @commands.command(name="Join_play", help="joins the channel the author is in", aliases=["join", "join_play", "j"])
    ...
    @commands.command(name="Leave", help="Leaves the channel the bot is currently in",
                      aliases=["leave", "L", "l", "dis", "Dis", "disconnect", "Disconnect"])
